...
namespace ProjektRoguelike
{
    ...
    public class Player : Entity
    {
        ...
        public override void Update()
        {
            ...
            // If the up key is pressed and the timer allows it, shoot.
            if (Globals.GetKey(Keys.Up) && timer.Test())
            {
                GameGlobals.PassProjectile(new BasicAttack(0 - 90, Position, null, 0 - 180, SpriteEffects.None));
                timer.ResetToZero();
            }
            // If the right key is pressed and the timer allows it, shoot.
            if (Globals.GetKey(Keys.Right) && timer.Test())
            {
                GameGlobals.PassProjectile(new BasicAttack(90 - 90, Position, null, 90 - 180, SpriteEffects.None));
                timer.ResetToZero();
            }
            // If the down key is pressed and the timer allows it, shoot.
            if (Globals.GetKey(Keys.Down) && timer.Test())
            {
                GameGlobals.PassProjectile(new BasicAttack(180 - 90, Position, null, 180 - 180, SpriteEffects.None));
                timer.ResetToZero();
            }
            // If the left key is pressed and the timer allows it, shoot.
            if (Globals.GetKey(Keys.Left) && timer.Test())
            {
                GameGlobals.PassProjectile(new BasicAttack(270 - 90, Position, null, 270 - 180, SpriteEffects.None));
                timer.ResetToZero();
            }
        }
    }
